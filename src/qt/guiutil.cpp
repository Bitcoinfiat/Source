...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bitcoinfiat address (e.g. %1)").arg("da4NsP8KutrV24MDQyYnhtS7AuqPLhWFRQ"));
#endif
...
{
    // return if URI is not valid or is no Bitcoinfiat: URI
...
{
    // Convert bitcoinfiat:// to bitcoinfiat:
    //
    //    Cannot handle this later, because bitcoinfiat:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open bitcoinfiat.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitcoinfiat.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Bitcoinfiat.lnk
...
{
    return GetAutostartDir() / "bitcoinfiat.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a bitcoinfiat.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Bitcoinfiat\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the bitcoinfiat app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add bitcoinfiat app to startup item list
