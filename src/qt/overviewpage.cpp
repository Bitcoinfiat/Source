...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::DEV)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sDEVPercentage, QString& szDEVPercentage)
{
    int nPrecision = 2;
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Percentage labels
    ui->labelDEVPercent->setText(sPercentage);
    ui->labelzDEVPercent->setText(szPercentage);
    ...
    if (fEnableZeromint) {
        ...
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in bitcoinfiat.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in bitcoinfiat.conf");
    }
    ...
    ui->labelzBalanceMature->setVisible(showzDEVAvailable);
    ui->labelzBalanceMatureText->setVisible(showzDEVAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzDEVUnconfirmed);
