...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBitcoinfiatAmount"))
        settings.setValue("nAnonymizeBitcoinfiatAmount", 1000);

    nAnonymizeBitcoinfiatAmount = settings.value("nAnonymizeBitcoinfiatAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBitcoinfiatAmount"))
        SoftSetArg("-anonymizebitcoinfiatamount", settings.value("nAnonymizeBitcoinfiatAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bitcoinfiat.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBitcoinfiatAmount:
            return QVariant(nAnonymizeBitcoinfiatAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBitcoinfiatAmount:
            nAnonymizeBitcoinfiatAmount = value.toInt();
            settings.setValue("nAnonymizeBitcoinfiatAmount", nAnonymizeBitcoinfiatAmount);
            emit anonymizeBitcoinfiatAmountChanged(nAnonymizeBitcoinfiatAmount);
            break;
        case CoinControlFeatures:
